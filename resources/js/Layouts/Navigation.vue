    ...
    <div :class="$page.props.showingMobileMenu ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
        ...
        <div class="flex justify-center items-center mt-8">
            <div class="flex items-center">
                ...
                <span class="mx-2 text-2xl font-semibold text-white">BlueRemesas</span>
            </div>
        </div>
        ...
        <nav class="mt-10" x-data="{ isMultiLevelMenuOpen: false }">
            ...
            <nav-link :href="route('contenedor.index')" :active="route().current('contenedor.index')">
                <i class="fa-solid fa-building"></i>
                Contenedores
            </nav-link>

            <nav-link :href="route('subcontenedor.index')" :active="route().current('subcontenedor.index')">
                <i class="fa-solid fa-building"></i>
                SubContenedores
            </nav-link>
            ...
            <nav-link :href="route('subproducto.index')" :active="route().current('subproducto.index')">
                <i class="fa-solid fa-user"></i>
                SubProductos         
            </nav-link>
                ...
                <div v-show="showingTwoLevelMenu">
                    <ul class="overflow-hidden p-2 mx-4 mt-2 space-y-2 text-sm font-medium text-white bg-gray-700 bg-opacity-50 rounded-md shadow-inner"
                        ...
                        <li class="px-2 py-1 transition-colors duration-150">
                            <Link class="w-full" :href="route('gastosusd.index')">Gastos USD </Link>
                        </li>
                    </ul>
                    <ul class="overflow-hidden p-2 mx-4 mt-2 space-y-2 text-sm font-medium text-white bg-gray-700 bg-opacity-50 rounded-md shadow-inner"
                        aria-label="submenu">
                        <li class="px-2 py-1 transition-colors duration-150">
                            <Link class="w-full" :href="route('gastoscup.index')">Gastos CUP</Link>
                        </li>
                    </ul>
                </div>
            </transition>
            
            <nav-link :href="route('ventas.index')" :active="route().current('ventas.index')">
            ...
            <nav-link :href="route('reporte')" :active="route().current('reporte')">
